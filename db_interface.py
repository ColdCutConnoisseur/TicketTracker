...
def export_inventory_for_tax_year_to_csv(tax_year: int, file_path: str):
    engine = create_and_return_db_engine()

    start_date = datetime.datetime(tax_year, 1, 1)
    end_date = datetime.datetime(tax_year + 1, 1, 1)

    with Session(engine) as session:
        stmt = (
            select(Inventory)
            .where(Inventory.event_date >= start_date)
            .where(Inventory.event_date < end_date)
        )
        ...
        # Dynamically get column names from the model
        columns = Inventory.__table__.columns.keys()
...
if __name__ == "__main__":
    ...
    export_inventory_for_tax_year_to_csv(tax_year=2025, file_path="inventory_2025.csv")
    print("Done.")
